...
function Homepage() {
  ...
  /**
   * Loads the given XML file and extracts all records with selos from the Remessa node.
   * The records are then processed and added to the data array.
  ...
  async function loadData(file) {
    ...
    try {
      ...
      // Get all records with selos from the Remessa node
  ...
  /**
   * Extracts the desired data from a given XML node and returns it in a
   * structured object format. The data includes the Selo, Codigo, RCPN, RIT,
   * Protocolo, data de Entrada, Pago, Gratuito, Livro, Folha, Termo and some
   * Emolumentos data.
   *
   * @param {Element} child The XML node to extract data from
   * @param {boolean} isRCPN Whether the document is RCPN or not
   * @returns {object} The extracted data in a structured object format
   */
  ...
  /**
   * Returns the code for a given XML node.
   *
   * @param {Element} child The XML node to get the code for
   * @returns {string} The code for the given XML node
   */
